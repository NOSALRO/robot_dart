...
namespace robot_dart {
    ...
    namespace robots {
        ...
        Robot::Robot(const std::string& model_file, const std::string& robot_name,
            bool is_urdf_string, bool cast_shadows, std::vector<RobotDamage> damages)
        ...
        {
            ROBOT_DART_ASSERT(
                index < _controllers.size(), "Controller index out of bounds", nullptr);
        ...
        void Robot::set_actuator_type(size_t dof, dart::dynamics::Joint::ActuatorType type,
            bool override_mimic, bool override_base)
        {
        ...
        void Robot::set_actuator_types(
            const std::vector<dart::dynamics::Joint::ActuatorType>& types, bool override_mimic,
            bool override_base)
        {
            ROBOT_DART_ASSERT(types.size() == _skeleton->getNumDofs(),
                "Actuator types vector size is not the same as the DOFs of the robot", );
        ...
        void Robot::set_actuator_types(
            dart::dynamics::Joint::ActuatorType type, bool override_mimic, bool override_base)
        {
            // Ignore first 6 dofs if robot is free, and override_base is false
            size_t start_dof = 0;
            if (free() && !override_base)
        ...
        {
            ROBOT_DART_ASSERT(dof < _skeleton->getNumDofs(), "DOF index out of bounds",
                dart::dynamics::Joint::ActuatorType::FORCE);
        ...
        void Robot::set_control_mode(const std::string& mode,
            const std::vector<std::string>& dof_names, bool override_mimic, bool override_base)
        {
            // Set all dofs to same control mode
            if (dof_names.empty()) {
                if (mode == "torque") {
                    return set_actuator_types(
                        dart::dynamics::Joint::FORCE, override_mimic, override_base);
                }
                else if (mode == "servo") {
                    return set_actuator_types(
                        dart::dynamics::Joint::SERVO, override_mimic, override_base);
                }
                else if (mode == "velocity") {
                    return set_actuator_types(
                        dart::dynamics::Joint::VELOCITY, override_mimic, override_base);
                }
                else if (mode == "passive") {
                    return set_actuator_types(
                        dart::dynamics::Joint::PASSIVE, override_mimic, override_base);
                }
                else if (mode == "locked") {
                    return set_actuator_types(
                        dart::dynamics::Joint::LOCKED, override_mimic, override_base);
                }
                else if (mode == "mimic") {
                    ROBOT_DART_WARNING(true,
                        "Use this only if you know what you are doing. Use set_mimic otherwise.");
                    return set_actuator_types(
                        dart::dynamics::Joint::MIMIC, override_mimic, override_base);
                }
                ROBOT_DART_EXCEPTION_ASSERT(
                    false, "Unknown type of control mode. Valid values: torque, servo, velocity");
            }

            for (size_t i = 0; i < dof_names.size(); i++) {
                auto it = _dof_map.find(dof_names[i]);
                ROBOT_DART_ASSERT(it != _dof_map.end(),
                    "set_control_mode: " + dof_names[i] + " is not in dof_map", );
                if (mode == "torque") {
                    set_actuator_type(
                        it->second, dart::dynamics::Joint::FORCE, override_mimic, override_base);
                }
                else if (mode == "servo") {
                    set_actuator_type(
                        it->second, dart::dynamics::Joint::SERVO, override_mimic, override_base);
                }
                else if (mode == "velocity") {
                    set_actuator_type(
                        it->second, dart::dynamics::Joint::VELOCITY, override_mimic, override_base);
                }
                else if (mode == "passive") {
                    set_actuator_type(
                        it->second, dart::dynamics::Joint::PASSIVE, override_mimic, override_base);
                }
                else if (mode == "locked") {
                    set_actuator_type(
                        it->second, dart::dynamics::Joint::LOCKED, override_mimic, override_base);
                }
                else if (mode == "mimic") {
                    ROBOT_DART_WARNING(true,
                        "Use this only if you know what you are doing. Use set_mimic otherwise.");
                    set_actuator_type(
                        it->second, dart::dynamics::Joint::MIMIC, override_mimic, override_base);
                }
                else
                    ROBOT_DART_EXCEPTION_ASSERT(false,
                        "Unknown type of control mode. Valid values: torque, servo, velocity");
            }
        }

        void Robot::set_mimic(const std::string& dof_name, const std::string& mimic_dof_name,
        ...
        std::string Robot::control_mode(const std::string& dof_name) const
        {
            ...
            ROBOT_DART_ASSERT(it != _dof_map.end(),
                "control_mode: " + dof_name + " is not in dof_map", "invalid");
        ...
        std::vector<std::string> Robot::control_modes(
            const std::vector<std::string>& dof_names) const
        {
            ...
            if (dof_names.empty()) {
                ...
                ROBOT_DART_ASSERT(str_modes.size() == modes.size(),
                    "control_modes: sizes of retrieved modes do not match", {});
            ...
            ROBOT_DART_ASSERT(str_modes.size() == dof_names.size(),
                "control_modes: sizes of retrieved modes do not match", {});
        ...
        {
            ROBOT_DART_ASSERT(
                joint_index < _skeleton->getNumJoints(), "Joint index out of bounds", {});
            ...
            if (child_body)
                F2 = -dart::math::dAdT(
                    jt->getTransformFromChildBodyNode(), child_body->getBodyForce());
        ...
        void Robot::set_external_torque(
            size_t body_index, const Eigen::Vector3d& torque, bool local)
        {
            ROBOT_DART_ASSERT(
                body_index < _skeleton->getNumBodyNodes(), "BodyNode index out of bounds", );
            auto bd = _skeleton->getBodyNode(body_index);
        ...
        {
            ROBOT_DART_ASSERT(body_index < _skeleton->getNumBodyNodes(),
                "BodyNode index out of bounds", Eigen::Vector6d::Zero());
            auto bd = _skeleton->getBodyNode(body_index);
        ...
        {
            ROBOT_DART_ASSERT(body_index < _skeleton->getNumBodyNodes(),
                "BodyNode index out of bounds", Eigen::Isometry3d::Identity());
            ...
            if (_joint_map.empty()) {
                ...
                for (size_t i = 0; i < _skeleton->getNumJoints(); i++)
                    if (_skeleton->getJoint(i)->getName() == joint_name)
                        return i;
                ROBOT_DART_ASSERT(
                    false, "joint_index : " + joint_name + " is not in the skeleton", 0);
            }
                ...
                if (extension == ".urdf") {
                    ...
                    if (!tmp_skel) {
                        dart::simulation::WorldPtr world
                            = dart::io::SkelParser::readWorld(model_file);
        ...
        std::shared_ptr<Robot> Robot::create_box(const Eigen::Vector3d& dims,
            const Eigen::Vector6d& pose, const std::string& type, double mass,
            const Eigen::Vector4d& color, const std::string& box_name)
        {
            ...
            if (type == "free")
                body = box_skel->createJointAndBodyNodePair<dart::dynamics::FreeJoint>(nullptr)
                           .second;
            else
                body = box_skel->createJointAndBodyNodePair<dart::dynamics::WeldJoint>(nullptr)
                           .second;
            ...
            const Eigen::Vector4d& color, const std::string& ellipsoid_name)
        {
            ...
            if (type == "free")
                body
                    = ellipsoid_skel->createJointAndBodyNodePair<dart::dynamics::FreeJoint>(nullptr)
                          .second;
            else
                body
                    = ellipsoid_skel->createJointAndBodyNodePair<dart::dynamics::WeldJoint>(nullptr)
                          .second;
