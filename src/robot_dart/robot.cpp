...
namespace robot_dart {
    ...
    void Robot::set_control_mode(const std::string& mode, const std::vector<std::string>& dof_names,
        bool override_mimic, bool override_base)
    {
        ...
        if (dof_names.empty()) {
            if (mode == "torque") {
                ...
                ROBOT_DART_WARNING(
                    true, "Use this only if you know what you are doing. Use set_mimic otherwise.");
                return set_actuator_types(
        ...
        for (size_t i = 0; i < dof_names.size(); i++) {
            ...
            ROBOT_DART_ASSERT(
                it != _dof_map.end(), "set_control_mode: " + dof_names[i] + " is not in dof_map", );
            if (mode == "torque") {
                set_actuator_type(
    ...
    std::string Robot::control_mode(const std::string& dof_name) const
    {
        ...
        ROBOT_DART_ASSERT(
            it != _dof_map.end(), "control_mode: " + dof_name + " is not in dof_map", "invalid");
